...
DECLARE
  ...
  v_allowed_q_keys text[] := ARRAY['question_number', 'question_text', 'translation_vi', 'options', 'options_vi', 'evidence'];
...
BEGIN
  -- =========================================================================
  -- PHASE A: VALIDATION ONLY (ZERO MUTATIONS / ZERO UPDATE STATEMENTS)
  -- =========================================================================
  ...
  IF p_payload ? 'documents' THEN
    v_effective_docs := p_payload->'documents';
    IF jsonb_typeof(v_effective_docs) <> 'null' THEN
      IF jsonb_typeof(v_effective_docs) <> 'array' THEN
        RETURN jsonb_build_object('success', false, 'error', 'documents must be null or a JSON array.');
      END IF;
      FOR v_doc_elem IN SELECT * FROM jsonb_array_elements(v_effective_docs)
      LOOP
    ...
    END IF;
  ELSE
    v_effective_docs := v_existing_docs;
  END IF;

  -- 6. Compute & Validate Effective Documents VI
  IF p_payload ? 'documents_vi' THEN
    v_effective_docs_vi := p_payload->'documents_vi';
    IF jsonb_typeof(v_effective_docs_vi) <> 'null' THEN
      IF jsonb_typeof(v_effective_docs_vi) <> 'array' THEN
        RETURN jsonb_build_object('success', false, 'error', 'documents_vi must be null or a JSON array.');
      END IF;
      FOR v_doc_elem IN SELECT * FROM jsonb_array_elements(v_effective_docs_vi)
      LOOP
        IF jsonb_typeof(v_doc_elem) <> 'object' THEN
        ...
        END IF;
        IF (v_doc_elem ? 'type') AND jsonb_typeof(v_doc_elem->'type') <> 'string' THEN
          RETURN jsonb_build_object('success', false, 'error', 'documents_vi type must be a string.');
        END IF;
        IF (v_doc_elem ? 'title') AND jsonb_typeof(v_doc_elem->'title') <> 'string' THEN
          RETURN jsonb_build_object('success', false, 'error', 'documents_vi title must be a string.');
        END IF;
      END LOOP;
    END IF;
  ELSE
    v_effective_docs_vi := v_existing_docs_vi;
  END IF;

  -- 7. Compute & Validate Effective Bilingual Units
  IF jsonb_typeof(v_effective_docs) = 'array' THEN
    v_max_doc_idx := jsonb_array_length(v_effective_docs);
  ELSE
    v_max_doc_idx := 0;
  END IF;

  IF p_payload ? 'part7_bilingual_units' THEN
    v_effective_units := p_payload->'part7_bilingual_units';
    IF jsonb_typeof(v_effective_units) <> 'null' THEN
      ...
      LOOP
        ...
        -- Validate Integer document_index
        IF NOT (v_unit_elem ? 'document_index') OR jsonb_typeof(v_unit_elem->'document_index') <> 'number' OR (v_unit_elem->>'document_index') !~ '^[0-9]+$' THEN
          RETURN jsonb_build_object('success', false, 'error', 'unit document_index must be a non-fractional integer >= 0.');
        END IF;
        v_doc_idx := (v_unit_elem->>'document_index')::int;
        IF v_doc_idx >= v_max_doc_idx THEN
          RETURN jsonb_build_object('success', false, 'error', format('unit document_index %s out of bounds (max %s).', v_doc_idx, v_max_doc_idx - 1));
        END IF;

        -- Validate Integer order
        IF NOT (v_unit_elem ? 'order') OR jsonb_typeof(v_unit_elem->'order') <> 'number' OR (v_unit_elem->>'order') !~ '^[0-9]+$' THEN
          RETURN jsonb_build_object('success', false, 'error', 'unit order must be a non-fractional integer >= 0.');
        END IF;
  ...
  IF p_payload ? 'questions' THEN
    ...
    LOOP
      ...
      -- Check allowed question keys
      FOR v_q_key IN SELECT jsonb_object_keys(v_q_elem)
      LOOP
        IF NOT (v_q_key = ANY(v_allowed_q_keys)) THEN
          RETURN jsonb_build_object('success', false, 'error', format('Unsupported question key: %s', v_q_key));
        END IF;
      END LOOP;

      IF NOT (v_q_elem ? 'question_number') OR jsonb_typeof(v_q_elem->'question_number') <> 'number' OR (v_q_elem->>'question_number') !~ '^[0-9]+$' THEN
        RETURN jsonb_build_object('success', false, 'error', 'Question entry requires a non-fractional integer question_number.');
      END IF;
      ...
      IF v_q_elem ? 'evidence' THEN
        ...
        IF jsonb_typeof(v_ev) <> 'null' AND jsonb_typeof(v_ev) <> 'array' THEN
  ...
  LOOP
    v_patched_q := NULL;
    IF p_payload ? 'questions' THEN
      SELECT elem INTO v_patched_q 
      FROM jsonb_array_elements(p_payload->'questions') elem 
      WHERE (elem->>'question_number')::int = v_db_q.question_number;
    END IF;

    IF v_patched_q IS NOT NULL AND (v_patched_q ? 'evidence') THEN
      v_eff_ev := v_patched_q->'evidence';
    ELSE
    ...
    IF v_eff_ev IS NOT NULL AND jsonb_typeof(v_eff_ev) = 'array' AND jsonb_array_length(v_eff_ev) > 0 THEN
      ...
      FOR v_ev_item IN SELECT * FROM jsonb_array_elements(v_eff_ev)
      LOOP
        IF jsonb_typeof(v_ev_item) <> 'object' OR NOT (v_ev_item ? 'unit_id') THEN
          RETURN jsonb_build_object('success', false, 'error', format('Each evidence item for Q%s must be an object with unit_id.', v_db_q.question_number));
        END IF;
        IF jsonb_typeof(v_ev_item->'unit_id') <> 'string' OR TRIM(v_ev_item->>'unit_id') = '' THEN
          RETURN jsonb_build_object('success', false, 'error', format('Evidence unit_id in Q%s must be a non-empty string.', v_db_q.question_number));
        END IF;

        v_ev_unit_id := TRIM(v_ev_item->>'unit_id');
        IF NOT (v_ev_unit_id = ANY(v_unit_ids)) THEN
          RETURN jsonb_build_object('success', false, 'error', format('Evidence unit_id "%s" in Q%s does not exist in bilingual units of this group.', v_ev_unit_id, v_db_q.question_number));
        END IF;
      END LOOP;
    END IF;
  END LOOP;
  ...
  -- =========================================================================
  -- PHASE B: MUTATION PHASE (ONLY EXECUTES AFTER ALL VALIDATION SUCCEEDS)
  -- =========================================================================
    ...
    LOOP
      ...
      IF v_q_elem ? 'question_text' THEN
        ...
        SET question_text = CASE WHEN jsonb_typeof(v_q_elem->'question_text') = 'null' THEN NULL ELSE v_q_elem->>'question_text' END,
            updated_at = NOW()
        WHERE group_id = p_group_id AND question_number = v_q_num AND test_id = p_test_id AND LOWER(TRIM(part)) = 'part7';
      END IF;
      ...
      IF v_q_elem ? 'options' THEN
        UPDATE public.toeic_test_questions
        SET options = v_q_elem->'options', updated_at = NOW()
        WHERE group_id = p_group_id AND question_number = v_q_num AND test_id = p_test_id AND LOWER(TRIM(part)) = 'part7';
      END IF;

      IF v_q_elem ? 'options_vi' THEN
        UPDATE public.toeic_test_questions
        SET options_vi = v_q_elem->'options_vi', updated_at = NOW()
        WHERE group_id = p_group_id AND question_number = v_q_num AND test_id = p_test_id AND LOWER(TRIM(part)) = 'part7';
      END IF;

      IF v_q_elem ? 'evidence' THEN
        UPDATE public.toeic_test_questions
        SET evidence = CASE WHEN jsonb_typeof(v_q_elem->'evidence') = 'null' THEN NULL ELSE v_q_elem->'evidence' END,
            updated_at = NOW()
        WHERE group_id = p_group_id AND question_number = v_q_num AND test_id = p_test_id AND LOWER(TRIM(part)) = 'part7';
      END IF;
    END LOOP;
