...
import {
  getAdminReadingLesson,
  getAdminReadingQuestions,
  createReadingLesson,
  updateReadingLesson,
  saveReadingQuestion,
  hasReadingLessonHistory,
} from '../lib/supabase/adminReading';
import {
  validateReadingLessonDraft,
  validateReadingLessonForPublish,
  hasMaterialPassageChange,
  ReadingQuestionInput,
} from '../lib/cms/readingValidation';
import { slugifyTitle } from '../lib/cms/vocabularyValidation';
import { LearningLesson } from '../lib/supabase/types';
import { ReadingPreviewModal } from '../components/admin/ReadingPreviewModal';

export const AdminReadingLessonEditPage: React.FC = () => {
  ...
  const [toeicPart, setToeicPart] = useState<string>('part5');
  ...
  useEffect(() => {
    if (isEditing && lessonId) {
      async function loadLesson() {
        setLoading(true);
        const res = await getAdminReadingLesson(lessonId!);
        if (res.error || !res.data) {
          setFormError(res.error || 'Không tìm thấy bài học Reading.');
        } else {
          const l = res.data;
          setOriginalLesson(l);
          setTitle(l.title);
          ...
          setToeicPart(l.toeic_part || 'part5');
          setPassage(l.passage || '');
          setSortOrder(l.sort_order || 1);
          setIsPublished(l.is_published);

          // Check if lesson has student history
          const histRes = await hasReadingLessonHistory(l.id);
          setHasHistory(histRes.hasHistory);

          const qRes = await getAdminReadingQuestions(l.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const cmsQuestions: ReadingQuestionInput[] = questions.map((q, idx) => ({
      id: q.id,
      question_text: q.question_text,
      options: q.options,
      correct_answer: q.correct_answer,
    ...
    if (isPublished) {
      const pubValidation = validateReadingLessonForPublish(payloadInput);
      ...
      const draftValidation = validateReadingLessonDraft(payloadInput);
    ...
    if (isEditing && lessonId) {
      const res = await updateReadingLesson(lessonId, {
        title,
        slug,
        level,
        toeic_part: toeicPart,
      ...
      if (res.error || !res.data) {
        setFormError(res.error || 'Không thể tạo bài Reading.');
        setSaving(false);
        return;
      }
    ...
    if (activeLessonId) {
      for (let i = 0; i < questions.length; i++) {
        const q = questions[i];
        await saveReadingQuestion(activeLessonId, isPublished, {
          id: q.id,
          question_text: q.question_text,
          options: q.options,
          correct_answer: q.correct_answer,
    ...
    navigate('/admin/content/reading');
  };

  if (loading) {
    return <LoadingState message="Đang tải bài học Reading..." />;
  }

  const isSlugChangedOnPublished =
    isEditing && originalLesson?.is_published && slug !== originalLesson.slug;

  const previewQuestions: ReadingQuestionInput[] = questions.map((q, idx) => ({
    id: q.id,
    question_text: q.question_text,
    options: q.options,
    correct_answer: q.correct_answer,
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          <ArrowLeft className="w-4 h-4" /> Quay lại Danh sách Reading
        </NavLink>

        <button
          type="button"
          onClick={() => setShowPreview(true)}
          className="px-3.5 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 font-extrabold text-xs rounded-xl flex items-center gap-1.5 transition-colors"
        >
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
        <div className="border-b border-slate-100 pb-4">
          <h1 className="text-xl font-extrabold text-slate-900 flex items-center gap-2">
            ...
            {isEditing ? 'Chỉnh Sửa Bài Luyện Đọc (Reading)' : 'Tạo Bài Luyện Đọc Mới'}
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            {isEditing ? `Chỉnh sửa: ${originalLesson?.title}` : 'Soạn bài đọc, nội dung passage và câu hỏi trắc nghiệm.'}
          </p>
        </div>
        ...
        {hasHistory && (
          <div className="p-3.5 bg-blue-50 border border-blue-200 text-blue-900 text-xs font-bold rounded-2xl flex items-center gap-2">
            <BookOpen className="w-4 h-4 text-blue-600 shrink-0" />
            <span>Bài đọc này đã có dữ liệu học viên. Thay đổi lớn nội dung passage hoặc TOEIC Part sẽ bị khóa để bảo vệ tiến độ.</span>
          </div>
        )}
        ...
        {isSlugChangedOnPublished && (
          <div className="p-4 bg-amber-50 border border-amber-200 text-amber-900 text-xs font-semibold rounded-2xl space-y-1">
            ...
            <p className="text-[11px] leading-relaxed opacity-90">
              Bài đọc này đã xuất bản. Đổi Slug có thể làm hỏng bookmark đường dẫn của học viên.
            </p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  placeholder="VD: Reading Part 5 — Sentence Completion #1"
                  value={title}
                  onChange={(e) => handleTitleChange(e.target.value)}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:border-blue-600"
                />
              ...
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  placeholder="VD: reading-part5-foundation-1"
                  value={slug}
                  onChange={(e) => setSlug(e.target.value)}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono text-slate-900 focus:outline-none focus:border-blue-600"
                />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  disabled={hasHistory}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:border-blue-600 disabled:opacity-60"
                >
                  <option value="part5">Part 5 (Sentence Completion)</option>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-extrabold text-slate-700 block">
                Nội Dung Đoạn Văn (Passage Text){' '}
                {(toeicPart === 'part6' || toeicPart === 'part7') && isPublished && (
                  <span className="text-rose-500">* (Bắt buộc cho Part 6 & Part 7)</span>
                )}
              </label>
              <textarea
                rows={6}
                placeholder="Dán toàn bộ bài đọc, email, bài báo hoặc văn bản Part 6/7 tại đây..."
                value={passage}
                onChange={(e) => setPassage(e.target.value)}
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-900 focus:outline-none focus:border-blue-600 whitespace-pre-wrap font-sans"
              />
              {errors.passage && <p className="text-[11px] font-bold text-rose-500">{errors.passage}</p>}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2">
              <h3 className="text-sm font-extrabold text-slate-900 flex items-center gap-2">
                <HelpCircle className="w-4 h-4 text-blue-600" /> 2. Câu Hỏi Trắc Nghiệm ({questions.length} câu)
              </h3>
              <button
                type="button"
                onClick={handleAddQuestion}
                className="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 font-extrabold text-xs rounded-xl flex items-center gap-1 transition-colors"
              >
                <Plus className="w-4 h-4" /> Thêm Câu Hỏi Reading
              </button>
            </div>
            ...
            {questions.length === 0 ? (
              <div className="p-6 bg-slate-50 border border-dashed border-slate-200 rounded-2xl text-center text-xs text-slate-400 font-medium">
                Chưa có câu hỏi trắc nghiệm nào. Bấm "+ Thêm Câu Hỏi Reading" để bắt đầu.
              </div>
            ) : (
                ...
                >
                  <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                    <div className="flex items-center gap-2">
                      <span className="font-extrabold text-xs text-blue-700">Câu Hỏi {qIdx + 1}</span>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:border-blue-600"
                    />
                  </div>
                    ...
                    {Array.from({ length: 4 }).map((_, optIdx) => (
                      <div key={optIdx} className="space-y-1">
                        ...
                        <input
                          ...
                          className="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium text-slate-900 focus:outline-none focus:border-blue-600"
                        />
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="space-y-1">
                      ...
                      <select
                        ...
                        className="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:border-blue-600"
                      >
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        type="text"
                        placeholder="VD: Word Forms / Main Idea"
                        value={q.skill_tag}
                        onChange={(e) =>
                          setQuestions(
                        ...
                        className="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-900 focus:outline-none focus:border-blue-600"
                      />
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        type="text"
                        placeholder="VD: Business / Marketing"
                        value={q.topic}
                        onChange={(e) =>
                          setQuestions(
                  ...
                  <div className="space-y-1">
                    <label className="text-[11px] font-extrabold text-slate-700 block">
                      Hình ảnh minh họa (Image URL tùy chọn)
                    </label>
                    <input
                      type="url"
                      placeholder="https://example.com/reading-chart.jpg"
                      value={q.image_url}
                      onChange={(e) =>
                        setQuestions(
                      ...
                      className="w-full px-3.5 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono text-slate-900 focus:outline-none focus:border-blue-600"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      type="text"
                      placeholder="VD: Đáp án A đúng vì trong đoạn 2 tác giả nhấn mạnh..."
                      value={q.explanation}
                      onChange={(e) =>
                        setQuestions(
          ...
          <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
            ...
            <button
              type="submit"
              disabled={saving}
              className="px-5 py-2.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-extrabold text-xs rounded-xl shadow-md shadow-blue-600/20 flex items-center gap-2 transition-all"
            >
