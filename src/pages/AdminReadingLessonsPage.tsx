...
import {
  getAdminReadingLessons,
  getAdminReadingQuestions,
  setReadingLessonPublished,
  AdminReadingLessonInfo,
  PaginatedReadingLessons,
...
export const AdminReadingLessonsPage: React.FC = () => {
  const [lessonsData, setLessonsData] = useState<PaginatedReadingLessons | null>(null);
  ...
  // Preview modal state
  const [previewLesson, setPreviewLesson] = useState<AdminReadingLessonInfo | null>(null);
  ...
  const loadData = async (targetPage: number = 1) => {
    ...
    const res = await getAdminReadingLessons({
      searchQuery,
      levelFilter,
      toeicPartFilter,
      statusFilter,
    ...
    if (res.error || !res.data) {
      setErrorMsg(res.error || 'Không thể tải danh sách Reading.');
      setLessonsData(null);
    } else {
  ...
  const handleOpenPreview = async (lesson: AdminReadingLessonInfo) => {
    const qRes = await getAdminReadingQuestions(lesson.id);
    setPreviewQuestions(qRes.data || []);
    setPreviewLesson(lesson);
  };

  const handleTogglePublish = async (lesson: AdminReadingLessonInfo) => {
    setUpdatingLessonId(lesson.id);
    const nextStatus = !lesson.is_published;
    const res = await setReadingLessonPublished(lesson.id, nextStatus);

    if (res.error) {
      setToastMsg(`Lỗi: ${res.error}`);
    } else {
      let msg = `Đã ${nextStatus ? 'xuất bản' : 'ẩn'} bài Reading "${lesson.title}".`;
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      <div className="flex items-center justify-between">
        ...
        <NavLink
          to="/admin/content/reading/new"
          className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white font-extrabold text-xs rounded-xl shadow-md shadow-blue-600/20 inline-flex items-center gap-1.5 transition-all"
        >
          <Plus className="w-4 h-4" /> Tạo Bài Reading Mới
        </NavLink>
      </div>
      ...
      <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-2">
        <h1 className="text-xl font-extrabold text-slate-900 flex items-center gap-2">
          <BookOpen className="w-6 h-6 text-blue-600" /> Quản Lý Reading (Reading CMS)
        </h1>
        <p className="text-xs text-slate-500 font-medium">
          Soạn thảo bài đọc TOEIC Part 5 (Sentence Completion), Part 6 (Text Completion), và Part 7 (Reading Comprehension) cùng các câu hỏi trắc nghiệm.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3">
        <form onSubmit={handleSearchSubmit} className="w-full sm:w-80 relative">
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-900 focus:outline-none focus:border-blue-600"
          />
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          ...
          <div className="flex items-center gap-1">
            ...
            <select
              ...
              className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:border-blue-600"
            >
          ...
          <div className="flex items-center gap-1">
            ...
            <select
              ...
              className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:border-blue-600"
            >
              <option value="all">Tất cả (Part 5-7)</option>
      ...
      {/* Main List */}
      {loading ? (
        <LoadingState message="Đang tải danh sách bài Reading..." />
      ) : errorMsg ? (
        ...
        <EmptyState
          ...
          description="Bấm nút 'Tạo Bài Reading Mới' phía trên để soạn bài đầu tiên."
          icon={BookOpen}
        />
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 border-b border-slate-100 text-slate-500 font-extrabold uppercase tracking-wider">
                <tr>
                  <th className="py-3.5 px-4">Bài đọc (Title & Slug)</th>
                  <th className="py-3.5 px-4">TOEIC Part</th>
                  <th className="py-3.5 px-4">Trình độ</th>
                  <th className="py-3.5 px-4">Passage Status</th>
              ...
              <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                {lessonsData.lessons.map((lesson) => (
                  <tr key={lesson.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      <span className="px-2.5 py-0.5 bg-blue-50 text-blue-700 text-[11px] font-extrabold uppercase rounded-lg border border-blue-100">
                        {lesson.toeic_part || 'Part 5'}
                      </span>
                    </td>
                    <td className="py-3.5 px-4">
                    ...
                    <td className="py-3.5 px-4 text-[11px]">
                      {lesson.passage && lesson.passage.trim() ? (
                        <span className="text-emerald-700 font-bold flex items-center gap-1">
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> Có Passage
                        </span>
                      ) : (
                        <span className="text-slate-400 italic">Không có passage</span>
                      )}
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right space-x-2 shrink-0">
                      <button
                        ...
                        className="px-2.5 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 font-extrabold text-[11px] rounded-lg transition-colors"
                      >
                        Preview
                      </button>

                      <NavLink
                        to={`/admin/content/reading/${lesson.id}/edit`}
