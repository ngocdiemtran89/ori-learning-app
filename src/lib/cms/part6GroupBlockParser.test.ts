...
131.
Choose the sentence that best completes blank 131.
...
132.
Choose the best word for blank 132.
(A) years
(B) space
(C) beauty
(D) moisture
...
133.
Choose the best word for blank 133.
(A) also
(B) rarely
(C) somehow
(D) nevertheless
...
134.
Choose the best word for blank 134.
(A) its
(B) our
(C) others
(D) their`;

  it('1. parses exact TOEIC Part 6 group text (passage + 4 questions + choices)', () => {
    ...
    expect(res.questions[0].questionText).toBe('Choose the sentence that best completes blank 131.');
    ...
    expect(res.questions[1].questionText).toBe('Choose the best word for blank 132.');
    expect(res.questions[1].options).toEqual(['years', 'space', 'beauty', 'moisture']);

    expect(res.missingQuestionNumbers.length).toBe(0);
  });

  it('2. parses questions without question_text (optional stem test)', () => {
    const optionalStemText = `Passage content here... ------- 131.\n\n132.\n(A) years\n(B) space\n(C) beauty\n(D) moisture\n`;
    const res = parsePart6GroupBlock(optionalStemText, 131, 134);

    const q132 = res.questions.find(q => q.question_number === 132);
    expect(q132).toBeDefined();
    expect(q132?.questionText).toBe('');
    expect(q132?.options).toEqual(['years', 'space', 'beauty', 'moisture']);
  });
