...
export function getPartSummary(questions: Array<{ part: string; question_number: number; is_active?: boolean }>) {
  ...
  const summary: Record<CanonicalToeicPart, { count: number; expected: number; isComplete: boolean }> = {
    part1: { count: 0, expected: 6, isComplete: false },
    part2: { count: 0, expected: 25, isComplete: false },
    part3: { count: 0, expected: 39, isComplete: false },
    part4: { count: 0, expected: 30, isComplete: false },
    part5: { count: 0, expected: 30, isComplete: false },
    part6: { count: 0, expected: 16, isComplete: false },
    part7: { count: 0, expected: 54, isComplete: false },
  };
  ...
  CANONICAL_TOEIC_PARTS.forEach((partKey) => {
    summary[partKey].isComplete = summary[partKey].count === summary[partKey].expected;
  });
